...
from app.api.deps import get_current_active_admin, SessionDeps
...
def delete_teacher(db: SessionDeps, teacher_id: UUID) -> Message:
    ...
    if result:
        return Message(message="Teacher deleted successfully")
    raise  HTTPException(
        status_code=404,
...
def get_teacher_route(db: SessionDeps, teacher_id: UUID):
    teacher_db = teacher.get_teacher(db, teacher_id)
    if not teacher_db:
        raise HTTPException(
            status_code=404,
            detail="Teacher not found"
